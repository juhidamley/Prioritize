...
// Create multi-backend for both mouse and touch support
...
export function QueueView() {
  ...
  const handleAddTask = () => {
    if (newTaskTitle.trim()) {
      store.addTask(queue.id, newTaskTitle, newTaskDescription || undefined, newTaskColor || undefined);
      setNewTaskTitle('');
  ...
  const handleDeleteQueue = () => {
    if (confirm(`Are you sure you want to delete "${queue.name}"?`)) {
      ...
      if (queue.parentTaskId) {
        // Navigate back to parent queue
        const parentQueue = store.getState().queues.find(q => 
          q.tasks.some(t => t.id === queue.parentTaskId)
        );
        if (parentQueue) {
          navigate(`/queue/${parentQueue.id}`);
        } else {
          navigate('/');
        }
      } else {
            ...
            <div className="max-w-3xl mx-auto space-y-3">
              {queue.tasks.map((task, index) => (
                <TaskCard
                  ...
                  onOpenSubQueue={handleOpenSubQueue}
                  hasSubQueue={!!task.subQueueId}
                />
              ))}
            </div>
          )}
            ...
            <div className="space-y-4 py-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewTaskTitle(e.target.value)}
                  placeholder="Enter task title"
                  onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && handleEditTask()}
                />
              </div>
              <div>
